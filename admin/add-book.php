...
if(!empty($_FILES["bookpdf"]["name"]))
{
    ...
    $pdf_extension = strtolower(pathinfo($bookpdf, PATHINFO_EXTENSION));
    if($pdf_extension != 'pdf')
    {
        $pdf_error = true;
        echo "<script>alert('Invalid eBook format. Only .pdf files are allowed.');</script>";
    }
    else
    {
...
{
echo "<script>alert('Invalid format. Only jpg / jpeg/ png /gif format allowed for Book Picture');</script>";
}
elseif(!$pdf_error)
{
move_uploaded_file($_FILES["bookpic"]["tmp_name"],"bookimg/".$imgnewname);
$sql="INSERT INTO  tblbooks(BookName,CatId,AuthorId,ISBNNumber,BookPrice,bookImage,bookPdf,bookQty) VALUES(:bookname,:category,:author,:isbn,:price,:imgnewname,:pdfnewname,:bqty)";
...
<div class="col-md-6">  
 <div class="form-group">
 <label>Book PDF (eBook for Online Reading) <small class="text-muted">(.pdf format, optional)</small></label>
 <input class="form-control" type="file" name="bookpdf" accept=".pdf" autocomplete="off" />
 </div>
</div>
